...
import {HsCodeResponse, EstimateResponse} from './openai-response.interface';
import {ProductInfoRequestDto} from './openai-request.dto';

//********************************************************************************************************************

/**
 * OpenAI 서비스 - Chat Completions API 사용 (권장 방식)
 */
@Injectable()
...
Image: ${imageUrl}`;
      ...
      this.logger.log(`HSCode retrieved for product: ${productName}`);
      ...
      this.logger.error(`Failed to get HSCode for product: ${productName}`, error);
      throw error;
    }
  }
  ...
  async getWeightVolume(params: ProductInfoRequestDto): Promise<EstimateResponse> {
    ...
    try {
      //       const systemPrompt = `You are an expert in product dimensions and weight estimation.
      // Analyze the product information and return accurate volume and weight data in JSON format.
      // Your response must be a valid JSON object with these exact fields:
      // - volume: string (dimensions in format "LxWxH", e.g., "20x40x2")
      // - packed_volume: string (packed dimensions in format "LxWxH", e.g., "10x20x2")
      // - weight: number (weight in kg, e.g., 0.6)
      // - reason: string (explanation for the estimation)
...
Image: ${imageUrl}`;
      ...
      this.logger.log(`Estimate retrieved for product: ${productName}`);
      ...
      this.logger.error(`Failed to get estimate for product: ${productName}`, error);
      throw error;
    }
  }
   ...
   */
  async getEstimateInfo(params: ProductInfoRequestDto): Promise<{
...
// location: sazo-korea-shop-product/src/business/api/openai/openai.service.ts
